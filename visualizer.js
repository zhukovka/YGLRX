...
export function initializeVisualizer (canvasElement) {
    try {
...
export function setupAudioApi (audioElement) {
    let src = audioContext.createMediaElementSource(audioElement);

    let audioAnalyserNode = audioContext.createAnalyser();
    ...
    audioAnalyserNodes.push(audioAnalyserNode);
...
function paint () {
    ...
    for (let audioAnalyserNode of audioAnalyserNodes) {
